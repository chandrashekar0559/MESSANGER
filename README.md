...
$(function() {
  ...
  function addParticipantsMessage (data) {
	  console.log(data)
	//  $('.userlist').remove($('<li>').text());
    var message = '';
	$('.getUser').remove();
	
    if (data.numUsers === 1) {
		console.log(data);
      //message += "No others in online.";
	  $('.userlist').append($('<li class="getUser">').text("No others in online."));
    } else {
		console.log(data);
     // message += "there are " + data.numUsers + " participants---"+data.addedUserName	;
	  for(i=0;i<data.addedUserName.length;i++){
		    $('.userlist').append($('<li class="getUser">').text(data.addedUserName[i]));
	  // $('.userlist').append($('<li>').text(data.addedUserName[i]));
	  }
    }
    log(message);
  }
